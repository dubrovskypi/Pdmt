    ...
    public class InsightsServiceTests
    {
        ...
        public InsightsServiceTests()
        {
            _db = new AppDbContext(new DbContextOptionsBuilder<AppDbContext>()
                ...
                .Options);
            _service = new InsightsService(_db, new ConfigurationBuilder()
        ...
        public async Task GetRepeatingTriggersAsync_OrderedByAvgIntensityDescending()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                _db.EventTags.Add(new EventTag { EventId = ev1.Id, TagId = tag1.Id });
                _db.EventTags.Add(new EventTag { EventId = ev2.Id, TagId = tag2.Id });
            }
            await _db.SaveChangesAsync();
        ...
        public async Task GetRepeatingTriggersAsync_NoEvents_ReturnsEmpty()
        {
            ...
            var result = await _service.GetRepeatingTriggersAsync(userId, now, now.AddDays(7), minCount: 3);
        ...
        public async Task GetRepeatingTriggersAsync_FiltersOutsideDateRange()
        {
            ...
            _db.Events.Add(outsideEv);
            _db.EventTags.Add(new EventTag { EventId = outsideEv.Id, TagId = tag.Id });
            await _db.SaveChangesAsync();
        ...
        public async Task GetRepeatingTriggersAsync_IsolatesByUserId()
        {
            ...
            var result = await _service.GetRepeatingTriggersAsync(userId1, now, now.AddDays(7), minCount: 3);
        ...
        public async Task GetNextDayEffectsAsync_TagOnExactly3Days_IsIncluded()
        {
            ...
            for (int i = 1; i < 4; i++)
                _db.Events.Add(new Event { Id = Guid.NewGuid(), UserId = userId, Timestamp = new DateTimeOffset(dayStart.AddDays(i), TimeSpan.Zero), Type = EventType.Positive, Title = $"Follow{i}", Intensity = 4 });
            await _db.SaveChangesAsync();
        ...
        public async Task GetNextDayEffectsAsync_OrderedByAbsNextDayScore()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                _db.Events.Add(ev);
                _db.EventTags.Add(new EventTag { EventId = ev.Id, TagId = tagA.Id });
            }
            ...
            for (int i = 3; i < 6; i++)
            {
                ...
                _db.Events.Add(ev);
                _db.EventTags.Add(new EventTag { EventId = ev.Id, TagId = tagB.Id });
            }
            // Add following day events with opposite valence
            for (int i = 1; i < 4; i++)
                _db.Events.Add(new Event { Id = Guid.NewGuid(), UserId = userId, Timestamp = new DateTimeOffset(dayStart.AddDays(i), TimeSpan.Zero), Type = EventType.Positive, Title = $"FollowPos{i}", Intensity = 8 });
            for (int i = 4; i < 7; i++)
                _db.Events.Add(new Event { Id = Guid.NewGuid(), UserId = userId, Timestamp = new DateTimeOffset(dayStart.AddDays(i), TimeSpan.Zero), Type = EventType.Negative, Title = $"FollowNeg{i}", Intensity = 2 });
            await _db.SaveChangesAsync();
        ...
        public async Task GetTagCombosAsync_PairOrdering_AlphabeticalKey()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                _db.Events.AddRange(ev1, ev2);
                _db.EventTags.Add(new EventTag { EventId = ev1.Id, TagId = tagZ.Id });
                _db.EventTags.Add(new EventTag { EventId = ev2.Id, TagId = tagA.Id });
            }
            await _db.SaveChangesAsync();
        ...
        public async Task GetTagCombosAsync_NoEvents_ReturnsEmpty()
        {
            ...
            var result = await _service.GetTagCombosAsync(userId, new DateTimeOffset(dayStart, TimeSpan.Zero), new DateTimeOffset(dayStart.AddDays(7), TimeSpan.Zero));
        ...
        public async Task GetTagTrendAsync_Week_GroupsByMonday()
        {
            ...
            _db.Events.AddRange(ev1, ev2);
            _db.EventTags.Add(new EventTag { EventId = ev1.Id, TagId = tag.Id });
            _db.EventTags.Add(new EventTag { EventId = ev2.Id, TagId = tag.Id });
            await _db.SaveChangesAsync();
        ...
        public async Task GetTrendsAsync_EmptyRange_ReturnsEmpty()
        {
            ...
            var result = await _service.GetTrendsAsync(userId, DateTimeOffset.UtcNow, DateTimeOffset.UtcNow.AddDays(30), Granularity.Week);
        ...
        public async Task GetInfluenceabilitySplitAsync_OnlyNegativeEvents_Considered()
        {
            ...
            for (int i = 0; i < 3; i++)
                _db.Events.Add(new Event { Id = Guid.NewGuid(), UserId = userId, Timestamp = now.AddHours(i), Type = EventType.Positive, Title = $"P{i}", Intensity = 8 });
            for (int i = 0; i < 2; i++)
                _db.Events.Add(new Event { Id = Guid.NewGuid(), UserId = userId, Timestamp = now.AddDays(1).AddHours(i), Type = EventType.Negative, Title = $"N{i}", Intensity = 5, CanInfluence = true });
            await _db.SaveChangesAsync();
        ...
        public async Task GetInfluenceabilitySplitAsync_Split_CalculatedCorrectly()
        {
            ...
            for (int i = 0; i < 3; i++)
                _db.Events.Add(new Event { Id = Guid.NewGuid(), UserId = userId, Timestamp = now.AddHours(i), Type = EventType.Negative, Title = $"Inf{i}", Intensity = 6, CanInfluence = true });
            // 2 not influenceable (intensity 4 each)
            for (int i = 0; i < 2; i++)
                _db.Events.Add(new Event { Id = Guid.NewGuid(), UserId = userId, Timestamp = now.AddHours(10 + i), Type = EventType.Negative, Title = $"NotInf{i}", Intensity = 4, CanInfluence = false });
            await _db.SaveChangesAsync();
        ...
        public async Task GetInfluenceabilitySplitAsync_AllCanInfluence_CannotIsZero()
        {
            ...
            var now = DateTimeOffset.UtcNow;
            for (int i = 0; i < 3; i++)
                _db.Events.Add(new Event { Id = Guid.NewGuid(), UserId = userId, Timestamp = now.AddHours(i), Type = EventType.Negative, Title = $"N{i}", Intensity = 6, CanInfluence = true });
            await _db.SaveChangesAsync();
        ...
        public async Task GetInfluenceabilitySplitAsync_NoNegativeEvents_AllZeros()
        {
            ...
            var result = await _service.GetInfluenceabilitySplitAsync(userId, now, now.AddDays(7));
        ...
        public async Task GetWeekdayStatsAsync_AlwaysReturnsSevenDays()
        {
            ...
            _db.Events.Add(new Event { Id = Guid.NewGuid(), UserId = userId, Timestamp = monday, Type = EventType.Positive, Title = "Mon", Intensity = 5 });
            _db.Events.Add(new Event { Id = Guid.NewGuid(), UserId = userId, Timestamp = wednesday, Type = EventType.Negative, Title = "Wed", Intensity = 3 });
            await _db.SaveChangesAsync();
        ...
        public async Task GetWeekdayStatsAsync_OrderIsMonToSun()
        {
            ...
            var result = await _service.GetWeekdayStatsAsync(userId, DateTimeOffset.UtcNow, DateTimeOffset.UtcNow.AddDays(30));
