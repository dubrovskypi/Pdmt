...
- **Sticky save bar (form pages):** `NewEventPage`/`EditEventPage` use `Grid RowDefinitions="*,Auto"` — `*` is the `ScrollView` form, `Auto` is the bottom action bar with a 1px top `BoxView` divider and a full-width filled button. No `ToolbarItem` for Save. Disabled state uses `SurfaceVariant`/`Muted`. Do **not** override Android keyboard handlers (`EditorHandler`/`EntryHandler` IME interception, custom `SoftInputExtensions`) — only the built-in MAUI `KeyboardScrollManager` is allowed.
